...
class OnTradeSpawn : ResourceHook {
	...
	Document doc("The inner hook is triggered when a civilian ship carrying this resource is spawned. This hook is run on the resource's native planet.");
	Argument hookID(AT_Hook, "bonus_effects::BonusEffect");

	bool instantiate() override {
		@hook = cast<BonusEffect>(parseHook(arguments[0].str, "bonus_effects::"));
		if(hook is null) {
			error("OnTradeSpawn(): could not find inner hook: "+escape(arguments[0].str));
			return false;
		}
		return ResourceHook::instantiate();
	}
