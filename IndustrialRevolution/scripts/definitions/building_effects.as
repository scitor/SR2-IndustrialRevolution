...
/*tidy final class ActiveWhenCargoClassConsumed : GenericEffect {
	Document doc("This hook applies while a particular amount of cargo can be consumed every interval.");
	Argument cargo_class(AT_Custom, doc="Class of cargo to consume.");
	Argument amount(AT_Decimal, doc="Amount of cargo to consume per interval.");
	Argument interval(AT_Decimal, doc="Interval to consume cargo at.");
	Argument hookID(AT_Hook, "planet_effects::GenericEffect");
	Argument inactive_status(AT_Status, EMPTY_DEFAULT, doc="Status to add when this building is inactive.");
...
#section server
	...
	void tick(Object& obj, SurfaceBuilding@ bld, double time) const {
		...
		uint bestAmount = 0;
		uint bestType = uint(-1);
		if(obj.hasCargo) {
			for(uint i = 0, cnt = getCargoTypeCount(); i < cnt; ++i) {
				auto@ cargo = getCargoType(i);
				uint cargoAmount = uint(obj.getCargoStored(cargo.id));
				if(cargo.cls != cargo_class.str || double(cargoAmount) < amount.decimal)
					continue;
				if(cargoAmount < bestAmount) {
					bestAmount = cargoAmount;
					bestType = cargo.id;
				}
			}
		}
		...
		if(bestType == uint(-1)) {
			...
			if(info.timer <= 0) {
				if(!obj.hasCargo) {
					...
					double consAmt = obj.consumeCargo(bestType, amount.decimal, partial=false);
					if(consAmt < amount.decimal - 0.001) {
						cond = false;
						info.timer += interval.decimal;
					}
